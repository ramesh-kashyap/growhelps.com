...
class Team extends Controller
{
    ...
    {  
      ...
      return view('upnl/direct_team')->with(['direct_team'=>$notes])->with('search', $search);
    }
    ...
    {  
      ...
      $ids=$this->my_level_team_count($user->id);
        ...
        $limit = $request->limit ? $request->limit : 10;
          ...
          $notes = User::whereIn('id',$ids)->orderBy('id', 'DESC');
       if($search <> null && $request->reset!="Reset"){
        $notes = $notes->where(function($q) use($search){
        ...
        return view('upnl/total_team')->with(['direct_team'=>$notes])->with('search', $search);
    }



      public function RightLeft(Request $request)
    {  
      $user=Auth::user();
      // print_r($user->username);die();
      $ids=$this->team_by_position($user->id,'Right');
       ...
       if($search <> null && $request->reset!="Reset"){
        ...
        return view('upnl/right_left')->with(['direct_team'=>$notes])->with('search', $search);
    }
   ...
   public function LeftTeam(Request $request)
    {  
      ...
      $ids=$this->team_by_position($user->id,'Left');
       ...
       if($search <> null && $request->reset!="Reset"){
        ...
        return view('upnl/left_team')->with(['direct_team'=>$notes])->with('search', $search);
    }
    ...
    public function genealogy(Request $request)
    {
       ...
       $suser = @$request->suser;
        ...
        $username = $user->username;
         ...
         if(!empty($user_id) && $user_id->id>$session_id)
         {
          $username= $suser;  
         }
         else
    ...
    return view('upnl.genealogy')->with(compact('childs_1','childs_2','childs_3','childs_4','childs_5','childs_6','mydata'));
    }
